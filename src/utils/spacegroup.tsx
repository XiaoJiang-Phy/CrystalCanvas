// [Overview: Space group symbol lookup and academic-style JSX renderer with overbars & subscripts.]
// Copyright (c) 2026 Xiao Jiang and CrystalCanvas Contributors
// SPDX-License-Identifier: MIT OR Apache-2.0
import React from 'react';

/**
 * All 230 space group short Hermann-Mauguin symbols.
 * Extracted from spglib (spg_database.c) with standard ITC notation.
 *
 * Notation convention in this table:
 *   `-N`  before a digit means inversion (overbar), e.g. "P-1" = P1̄
 *   `_N`  after a digit means screw-axis subscript, e.g. "P2_1" = P2₁
 */
const SPACEGROUP_SYMBOLS: Record<number, string> = {
    1: "P1", 2: "P-1",
    3: "P2", 4: "P2_1", 5: "C2",
    6: "Pm", 7: "Pc", 8: "Cm", 9: "Cc",
    10: "P2/m", 11: "P2_1/m", 12: "C2/m",
    13: "P2/c", 14: "P2_1/c", 15: "C2/c",
    16: "P222", 17: "P222_1", 18: "P2_12_12", 19: "P2_12_12_1",
    20: "C222_1", 21: "C222", 22: "F222", 23: "I222", 24: "I2_12_12_1",
    25: "Pmm2", 26: "Pmc2_1", 27: "Pcc2", 28: "Pma2", 29: "Pca2_1",
    30: "Pnc2", 31: "Pmn2_1", 32: "Pba2", 33: "Pna2_1", 34: "Pnn2",
    35: "Cmm2", 36: "Cmc2_1", 37: "Ccc2",
    ...
    200: "Pm-3", 201: "Pn-3", 202: "Fm-3", 203: "Fd-3",
    204: "Im-3", 205: "Pa-3", 206: "Ia-3",
...
/**
 * Get the raw short HM symbol string for a space group number.
 */
export function getSpacegroupSymbol(number: number): string {
    return SPACEGROUP_SYMBOLS[number] ?? `#${number}`;
}

/**
 * Render a space group symbol as a React element with proper academic styling:
 *   - `-N` (overbar): digit N with a top line (e.g., 3̄)
 *   - `_N` (subscript): digit N rendered as subscript (e.g., 2₁)
 *
 * Examples:
 *   "Fm-3m"    → Fm3̄m
 *   "P2_1/c"   → P2₁/c
 ...
 *   "P-42_1c"  → P4̄2₁c
 */
export function renderSpacegroupSymbol(number: number): React.ReactNode {
    const symbol = getSpacegroupSymbol(number);
    if (!symbol || symbol.startsWith('#')) {
        return <span>{symbol}</span>;
    }

    // Tokenize: split into segments of plain text, overbar digits, subscript digits
    // Pattern: -\d for overbar, _\d for subscript, everything else literal
    const tokens: React.ReactNode[] = [];
    let i = 0;
    let key = 0;

    while (i < symbol.length) {
        if (symbol[i] === '-' && i + 1 < symbol.length && /\d/.test(symbol[i + 1])) {
            // Overbar digit
            tokens.push(
                <span
                    key={key++}
            ...
            i += 2;
        } else if (symbol[i] === '_' && i + 1 < symbol.length && /\d/.test(symbol[i + 1])) {
            // Subscript digit
            tokens.push(
                <sub
                    key={key++}
                    style={{ fontSize: '0.7em', verticalAlign: 'sub', lineHeight: '0' }}
                >
                    {symbol[i + 1]}
                </sub>
            );
            ...
            while (
                i < symbol.length &&
                !(symbol[i] === '-' && i + 1 < symbol.length && /\d/.test(symbol[i + 1])) &&
                !(symbol[i] === '_' && i + 1 < symbol.length && /\d/.test(symbol[i + 1]))
            ) {
                plain += symbol[i];
                i++;
            }
            tokens.push(<span key={key++}>{plain}</span>);
        }
    }

    return <span style={{ whiteSpace: 'nowrap' }}>{tokens}</span>;
}
