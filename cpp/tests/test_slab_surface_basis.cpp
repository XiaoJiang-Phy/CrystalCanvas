...
ColMajorMatrix3d make_cubic_lattice(double a) {
    ColMajorMatrix3d L;
    L << a, 0, 0,
         0, a, 0,
         0, 0, a;
...
ColMajorMatrix3d make_fcc_lattice(double a) {
    ColMajorMatrix3d L;
    L << 0, a/2, a/2,
         a/2, 0, a/2,
         a/2, a/2, 0;
...
ColMajorMatrix3d make_bcc_lattice(double a) {
    ColMajorMatrix3d L;
    L << -a/2,  a/2,  a/2,
          a/2, -a/2,  a/2,
          a/2,  a/2, -a/2;
...
ColMajorMatrix3d make_hcp_lattice(double a, double c) {
    ColMajorMatrix3d L;
    L << a, -a/2, 0,
...
ColMajorMatrix3d make_pathological_lattice() {
...
ColMajorMatrix3i require_surface_basis(
    const ColMajorMatrix3d& lattice, int h, int k, int l)
{
    ColMajorMatrix3i output = ColMajorMatrix3i::Zero();
    EXPECT_TRUE(get_surface_basis(lattice, h, k, l, output));
    return output;
}

void verify_basis_invariants(const ColMajorMatrix3d& lattice, const ColMajorMatrix3i& P, int h, int k, int l) {
...
TEST(SlabSurfaceBasisTest, Cubic100) {
    ...
    auto P = require_surface_basis(L, 1, 0, 0);
    verify_basis_invariants(L, P, 1, 0, 0);
}

TEST(SlabSurfaceBasisTest, Cubic111) {
    auto L = make_cubic_lattice(5.0);
    auto P = require_surface_basis(L, 1, 1, 1);
...
TEST(SlabSurfaceBasisTest, Axis001) {
    auto L = make_cubic_lattice(5.0);
    auto P = require_surface_basis(L, 0, 0, 1);
    verify_basis_invariants(L, P, 0, 0, 1);
}

TEST(SlabSurfaceBasisTest, NormalizedIndices) {
    auto L = make_cubic_lattice(5.0);
    auto P_100 = require_surface_basis(L, 1, 0, 0);
    auto P_200 = require_surface_basis(L, 2, 0, 0);
...
TEST(SlabSurfaceBasisTest, ZeroIndexAttack) {
    auto L = make_cubic_lattice(5.0);
    ColMajorMatrix3i output = ColMajorMatrix3i::Constant(7);
    const ColMajorMatrix3i before = output;
    EXPECT_FALSE(get_surface_basis(L, 0, 0, 0, output));
    EXPECT_EQ(output, before);
}

TEST(SlabSurfaceBasisTest, LargeMillerIndex) {
    auto L = make_cubic_lattice(5.0);
    auto P = require_surface_basis(L, 7, 5, 3);
    verify_basis_invariants(L, P, 7, 5, 3);
}

TEST(SlabSurfaceBasisTest, SingularLattice) {
    ColMajorMatrix3d L = ColMajorMatrix3d::Zero();
    ColMajorMatrix3i output = ColMajorMatrix3i::Constant(7);
    EXPECT_FALSE(get_surface_basis(L, 1, 1, 1, output));
}
...
TEST(SlabSurfaceBasisTest, NonFiniteLattice) {
    auto L = make_cubic_lattice(5.0);
...
TEST(SlabSurfaceBasisTest, ObliqueLatticeStress) {
    ...
    auto P = require_surface_basis(L, 1, 1, 0);
