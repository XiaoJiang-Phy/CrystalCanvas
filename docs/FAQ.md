...
macOS Intel/Metal is the baseline development target. The `v0.8.0` release provides both Intel and Apple Silicon artifacts. Ubuntu/Vulkan is a secondary build and selected rendering-verification target; `v0.8.0` does not publish a Linux package. Windows support is deferred until the maintainer workflow requires it.
