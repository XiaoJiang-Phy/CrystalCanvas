...
use crystal_canvas::llm::provider::{create_provider, ProviderConfig};
...
async fn test_deepseek_live_api() {
    ...
    println!("==================================================");
    println!("Sending request to DeepSeek API ({})", config_model(&config));
    ...
    match response {
        Ok(res) => {
            ...
            // Parse test
            let json_result: Result<crystal_canvas::llm::command::CrystalCommand, _> = serde_json::from_str(&res);
            match json_result {
                Ok(cmd) => {
                ...
                Err(_) => {
                    ...
                    if let Some(json_block) = extract_markdown_json(&res) {
                        let block_result: Result<crystal_canvas::llm::command::CrystalCommand, _> = serde_json::from_str(&json_block);
                        if let Ok(cmd) = block_result {
                            println!("⚠️ Response was wrapped in markdown ```json ... ``` but parsed successfully!");
                            println!("Parsed: {:?}", cmd);
                        } else {
                            println!("❌ Response contained a markdown block but failed schema validation.");
                        }
                    } else {
                        println!("❌ Response was not clean JSON and no markdown code block found.");
                    }
                }
            }
        },
