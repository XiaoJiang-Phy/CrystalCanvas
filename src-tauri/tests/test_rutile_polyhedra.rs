...
fn test_rutile_tio6_octahedra() {
    ...
    let bond_analysis = state
        .bond_analysis
        .as_ref()
        .expect("Bond analysis should be computed");

    // Ti (only check intrinsic sites, as mirrored atoms don't have coordination info)
    let ti_indices: Vec<usize> = state
        .elements
        .iter()
    ...
    for &ti_idx in &ti_indices {
        ...
        assert_eq!(
            coord.coordination_number, 6,
            "Ti in Rutile should be 6-coordinated (TiO6)"
        );
        assert_eq!(
            coord.polyhedron_type, "Octahedron",
            "CN=6 should be classified as Octahedron"
        );
    }

    // Oxygen (only check intrinsic sites)
    let o_indices: Vec<usize> = state
        .elements
        .iter()
        .take(state.intrinsic_sites)
        .enumerate()
    ...
    for &o_idx in &o_indices {
        ...
        assert_eq!(
            coord.coordination_number, 3,
            "O in Rutile should be 3-coordinated"
        );
